...
df_schedule = pd.read_csv('dataset/Race_Schedule.csv')
df_results = pd.read_csv('dataset/Race_Results.csv')
...
race_dict = filtered_df_schedule.groupby(
    'year')['raceId'].apply(list).to_dict()
...
cumulative_victories = {
    constructor_id: 0 for constructor_id in constructor_ids}
...
for year, race_ids in race_dict.items():
    ...
    victories_per_constructor = filtered_results.groupby(
        'constructorId').size()
...
df_victories = pd.DataFrame.from_dict(
    constructor_victories, orient='index').fillna(0).astype(int)
...
df_victories = df_victories[['constructorId'] +
                            [col for col in df_victories.columns if col != 'constructorId']]
